...
**Note:** The above command is deprecated since it has been integrated with GenCache. To run LASER without GenCache, first set `global_cache.use_cache = False` in `LASER/config.yaml`, and then run
```bash
cd LASER
. ./long_running_expt.sh 500 "yes"
# first argument tells how many prompts to run the experiment on
```
